...
function getReactionGif($reaction,$minX,$minY,$aspectRatio,$scale,$flags=0,$format=array("png") ) { // $flags: 1: Color, 2: Identifier, 4: stöchiometrische Koeff, 8: SVG
	...
	$format_count=count_compat($format);
	if ($format_count==0) {
		return;
	}
	...
	$reaction["products"]+=0;
	if (!count_compat($reaction["molecules"]) || ($reaction["reactants"]<1 && $reaction["products"]<1) || count_compat($reaction["molecules"])!=$reaction["reactants"]+$reaction["products"]) {
	...
	for ($a=count_compat($reaction["molecules"])-1;$a>=0;$a--) {
		if (count_compat($reaction["molecules"][$a]["atoms"])>0) {
			$atomsFound=true;
		}
		else {
			// rausnehmen
	...
	$totalWidth+=(count_compat($reaction["molecules"])*2+1)*$margin+$arrowlength+$plussize*($reaction["reactants"]+$reaction["products"]-2); // Zwischenraum + Pfeil + Plus
	...
	$Xmargin=$margin;
	if ($totalWidth<$minX) {
		$Xmargin=($minX-$totalWidth)/(count_compat($reaction["molecules"])*2+1);
		$totalWidth=$minX;
	}
...
function paintMoleculeIntoImage(& $im,& $colorIndex,$molecule,& $xOffset,$yOffset,$xMin,$yMax,$iWidth,$iHeight,$scale,$molscale,$margin,$color,$Xmargin=0,$format="png") { // malt molekül in GIF
	...
	for ($a=0;$a<count_compat($molecule[BONDS]);$a++) {
		...
		case 2:
			...
			$neighbours1=count_compat($molecule["atoms"][$atom1][NEIGHBOURS]);
			$neighbours2=count_compat($molecule["atoms"][$atom2][NEIGHBOURS]);
			if (($neighbours1==1 && $neighbours2>=3) || ($neighbours1>=3 && $neighbours2==1) || $molecule["atoms"][$atom1][IS_CUMULENE] || $molecule["atoms"][$atom2][IS_CUMULENE]) { // also P / S
				...
				for ($b=0;$b<count_compat($molecule[BONDS][$a][RINGS]);$b++) {
	...
	for ($a=0;$a<count_compat($molecule["atoms"]);$a++) {
		...
		$fullPaint=($thisT!="C" || $charge!=0 || count_compat($molecule["atoms"][$a][NEIGHBOURS])==0 || $molecule["atoms"][$a][IS_CUMULENE] || $molecule["atoms"][$a][IS_ISOTOPE] || $radical!=0);
	...
	if (is_array($molecule[GROUPS])) foreach ($molecule[GROUPS] as $group) {
		...
		if ($group[GROUP_TYPE]=="SUP"
			&& !$group[EXPAND]
			&& !count_compat($group["repres_atoms"])) {
