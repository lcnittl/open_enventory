...
*/
	...
	if ($retval==0 || count_compat($a5)==0 || $a1==$a5) {
		return $retval;
	}
...
function getFake3DAtoms(& $molecule,$atom_no,$highest) {
	...
	for ($a=0;$a<count_compat($highest);$a++) {
	...
	for ($a=0;$a<count_compat($highest);$a++) {
		...
		for ($b=0;$b<count_compat($highest);$b++) {
			if ($a==$b) {
				continue;
			}
			$tempMolAtoms[ $highest[$b] ]["z"]-=$direction*$orientation/(count_compat($highest)); // -1
		}
	}
	return $tempMolAtoms;
}
...
function SMchiral(& $molecule,$atom_no) { // ,$from_atom_no
	...
	$neighboursCount=count_compat($neighbours);
